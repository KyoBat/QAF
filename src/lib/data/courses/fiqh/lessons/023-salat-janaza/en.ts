/**
 * Fiqh - Lesson 023: صلاة الجنازة (Funeral Prayer)
 * English Content - Multi-Madhhab Approach (Maliki Priority)
 */

export const content = `# 📜 صَلَاةُ الْجَنَازَةِ — The Funeral Prayer

> *"Pray over your dead"* — The funeral prayer is a supplication for the deceased and a collective obligation on the community.

---

## 📜 Foundational Texts (النصوص الأساسية)

### Prophetic Hadith

> On the authority of **Abu Hurayra** رضي الله عنه that the Prophet ﷺ said:
> **"Whoever follows a funeral until prayer is offered will have one qirat (of reward), and whoever follows it until burial will have two qirat"**
> They asked: What are the two qirat? He said: **"Like two immense mountains"**
> — Al-Bukhari (1325)

> And he ﷺ said: **"Hasten with the funeral, for if it was righteous then you are advancing it to good, and if it was otherwise then you are putting evil off your necks"**
> — Al-Bukhari (1315)

### 📚 Referencing (التخريج)

| Source | Number | Ruling |
|--------|--------|--------|
| **Sahih Al-Bukhari** | 1325 | ✅ Authentic |
| **Sahih Al-Bukhari** | 1315 | ✅ Authentic |

---

## 🎓 Scholarly Sayings (أقوال العلماء)

> **Imam Malik** رحمه الله said in *Al-Mudawwana*:
> *"The funeral prayer is a supplication for the deceased, and Al-Fatiha is not recited in it."*

> **Ibn Rushd** رحمه الله said in *Bidayat Al-Mujtahid*:
> *"They agreed it is a collective obligation, but differed on reciting Al-Fatiha in it."*

---

## 🔍 Linguistic Analysis (التحليل اللغوي)

### Vocabulary Table

| Term | Root | Technical Definition |
|------|------|----------------------|
| **Al-Janaza (الجَنَازَة)** | ج-ن-ز | The deceased or their bier |
| **Al-Qirat (القِيرَاط)** | ق-ر-ط | A great unit of reward |
| **Ad-Dafn (الدَّفْن)** | د-ف-ن | Placing the deceased in the grave |
| **At-Takbir (التَّكْبِير)** | ك-ب-ر | Saying "Allahu Akbar" |

---

## 📚 Detailed Explanation (الشرح المفصل)

### 🔷 Ruling and Importance

The funeral prayer is a **collective obligation (fard kifaya)**: if some perform it, the rest are absolved.

| Category | Prayer |
|----------|--------|
| **Every Muslim** | ✅ Obligatory (fard kifaya) |
| **Miscarried child who showed life** | ✅ Prayed upon |
| **Martyr of battle** | ❌ No prayer or washing |
| **Non-Muslim** | ❌ No prayer |

### 🔷 Description of the Prayer

| Element | Description |
|---------|-------------|
| **No ruku' or sujud** | Standing prayer only |
| **4 Takbirat** | With du'a between each |
| **The Taslim** | One taslim according to Malikis |
| **No adhan or iqama** | Like Eid prayer |

### 🔷 The Four Takbirat

| Takbira | What is recited (Maliki) |
|---------|-------------------------|
| **1st** | Opening du'a (recommended) |
| **2nd** | Prayer upon the Prophet ﷺ |
| **3rd** | Du'a for the deceased |
| **4th** | General du'a, then taslim |

### 🔷 Du'as of the Funeral Prayer

**After the 2nd takbira:**

<div dir="rtl">

**اللَّهُمَّ صَلِّ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ، كَمَا صَلَّيْتَ عَلَى إِبْرَاهِيمَ وَعَلَى آلِ إِبْرَاهِيمَ، إِنَّكَ حَمِيدٌ مَجِيدٌ**

*O Allah, send blessings upon Muhammad and upon the family of Muhammad...*

</div>

**After the 3rd takbira:**

<div dir="rtl">

**اللَّهُمَّ اغْفِرْ لَهُ وَارْحَمْهُ، وَعَافِهِ وَاعْفُ عَنْهُ، وَأَكْرِمْ نُزُلَهُ، وَوَسِّعْ مُدْخَلَهُ، وَاغْسِلْهُ بِالْمَاءِ وَالثَّلْجِ وَالْبَرَدِ**

*O Allah, forgive him, have mercy on him, grant him well-being, pardon him...*

</div>

### 🔷 Imam's Position Relative to Deceased

| School | Man | Woman |
|--------|-----|-------|
| 🟡 **Maliki** | **Middle** | **Middle** |
| 🟢 Hanafi | Chest | Middle |
| 🔵 Shafi'i | Head | Middle |
| 🟣 Hanbali | Head | Middle |

---

## ⚖️ Points of View of the Schools (آراء المذاهب)

| Issue | 🟡 Maliki | 🟢 Hanafi | 🔵 Shafi'i | 🟣 Hanbali |
|-------|-----------|-----------|------------|------------|
| **Al-Fatiha** | **No Fatiha** | No Fatiha | Obligatory | Obligatory |
| **Taslim** | **One taslim** | One | Two | Two |
| **Raising hands** | **1st takbira only** | 1st only | Every takbira | Every takbira |
| **Prayer for absent** | **Not legitimate** | Not legitimate | Legitimate | Legitimate if not prayed upon |

---

## 🌍 Modern Applications (التطبيقات المعاصرة)

| Situation | Problem | Shariah Solution |
|-----------|---------|------------------|
| 🏥 **Morgue** | Delayed burial | Permissible for necessity, hastening better |
| ✈️ **Repatriation** | Long delay | Permissible if for benefit |
| 🕌 **Prayer in mosque** | Deceased inside | Better outside the mosque |
| 👥 **Multiple funerals** | Several deceased | Permissible to line them all and pray once |

---

## ⚠️ Common Mistakes (الأخطاء الشائعة)

| Mistake | ❌ What some do | ✅ The correct way |
|---------|-----------------|-------------------|
| **Reciting Fatiha** | Reciting it like regular prayer | No Fatiha in Maliki school |
| **Delaying burial** | Long wait for relatives | Hasten with the funeral |
| **Loud crying** | Wailing and screaming | Patience and tears only |
| **Not attending** | Avoiding funerals | Collective obligation with immense reward |

---

## 🎯 Contemporary Cases

### ✈️ Case 1: Praying Janaza in Absentia (Death Abroad)

> **Situation:** Mariam's father passed away in another country and was buried there. She couldn't travel for the funeral.

**❓ Can she pray over him from her country?**

<details>
<summary>📜 See the answer</summary>

**Analysis:**

| Criterion | 🟡 Maliki | 🔵 Shafi'i |
|-----------|---------|----------|
| Salat al-gha'ib | **Not legitimate** | Legitimate |
| Reason | Negus case was exceptional | Generalizable |

> 💡 **Rule:** According to Malikis, prayer for the absent is not legitimate because the Negus case was exceptional. According to Shafi'is, it is permitted. Mariam can make du'a for her father without a formal janaza prayer.

</details>

---

### 👥 Case 2: Multiple Deceased in One Prayer

> **Situation:** In an accident, three members of the same family died. The family wants to pray over them together.

**❓ Can one janaza prayer be performed for multiple deceased?**

<details>
<summary>📜 See the answer</summary>

**Analysis:**

| Criterion | Application |
|-----------|-------------|
| Permission | Yes, it is permitted |
| Arrangement | Bodies are lined up in front of the imam |
| Order | Men closer to imam, then women |
| Du'a | Made in plural form |

> 💡 **Rule:** It is permitted to pray over multiple deceased in one prayer. They are placed before the imam and du'a is adapted to plural ("Allahumma ghfir lahum").

</details>

---

### 👩 Case 3: Women Participating in Janaza Prayer

> **Situation:** Aisha wants to participate in her grandmother's funeral prayer at the mosque. Some tell her women shouldn't attend.

**❓ Can women participate in janaza prayer?**

<details>
<summary>📜 See the answer</summary>

**Analysis:**

| Criterion | Application |
|-----------|-------------|
| Permission | 🟡 Malikis: Permitted but discouraged |
| Reason | Risk of wailing and fitna |
| If she attends | Her prayer is valid |

> 💡 **Rule:** According to Malikis, women's participation in janaza is permitted but discouraged. If she attends modestly without wailing, her prayer is valid.

</details>

---

### 🏥 Case 4: Janaza Prayer at the Hospital Morgue

> **Situation:** Abdullah's body is at the hospital morgue. The family wants to pray janaza there before transport to the cemetery.

**❓ Can janaza be prayed at the hospital morgue?**

<details>
<summary>📜 See the answer</summary>

**Analysis:**

| Criterion | Application |
|-----------|-------------|
| Prayer location | No specific place required |
| Morgue | Permitted if the place is clean |
| Better | Musalla or mosque is preferred |

> 💡 **Rule:** Janaza prayer can be performed in any clean and decent place, including the morgue if necessary. Ideally, it should be done at the musalla or mosque.

</details>

---

### ⚠️ Case 5: Praying for Someone Who Didn't Pray

> **Situation:** Ahmed died while he wasn't praying regularly. His family asks if they can pray over him.

**❓ Can janaza be prayed for a Muslim who didn't pray?**

<details>
<summary>📜 See the answer</summary>

**Analysis:**

| Criterion | Application |
|-----------|-------------|
| If he testified shahada | He is prayed upon |
| Sin of abandonment | Doesn't exclude from prayer |
| Du'a | Ask forgiveness for him |
| Denying obligation | Different case (kufr) |

> 💡 **Rule:** As long as he testified the shahada and didn't deny the obligation of prayer, he is prayed upon. Abandonment due to negligence is a grave sin but doesn't exit Islam according to the majority.

</details>

---

## 💭 Reflection

> *"Death is a reality we must face. The janaza prayer is our final favor to our deceased brothers and sisters. Let us not miss this opportunity to earn the immense reward of 'two mountains'."*

**Questions to ponder:**
1. Have I memorized the du'a for the deceased?
2. Do I attend funerals when I can?
3. Am I prepared for my own death?

---

## ❓ Frequently Asked Questions (الأسئلة الشائعة)

**Q1: Is Al-Fatiha recited in funeral prayer?**
> 🟡 In Maliki school: No, the funeral prayer is supplication, not a regular prayer.

**Q2: How many takbirat in funeral prayer?**
> Four takbirat by consensus of the scholars.

**Q3: Where does the imam stand relative to the deceased?**
> 🟡 In Maliki school: At the middle of the body for both men and women.

**Q4: Is prayer for the absent permissible?**
> 🟡 In Maliki school: No, prayer for the absent is not legitimate.

---

## 🎯 Practical Action (العمل التطبيقي)

**When attending a funeral:**

1. **Attend and seek reward** — A qirat is like an immense mountain
2. **Pray four takbirat** — With the transmitted supplications
3. **Make du'a for the deceased** — For forgiveness and mercy
4. **Follow until burial** — To earn two qirat
5. **Console the family** — With kind words and supplication

> 💡 **Reminder:** Hastening with the funeral is a prophetic sunna.

---

## 📝 Supplication for the Deceased (دعاء للميت)

<div dir="rtl">

**اللَّهُمَّ اغْفِرْ لِحَيِّنَا وَمَيِّتِنَا، وَشَاهِدِنَا وَغَائِبِنَا، وَصَغِيرِنَا وَكَبِيرِنَا، وَذَكَرِنَا وَأُنْثَانَا، اللَّهُمَّ مَنْ أَحْيَيْتَهُ مِنَّا فَأَحْيِهِ عَلَى الْإِسْلَامِ، وَمَنْ تَوَفَّيْتَهُ مِنَّا فَتَوَفَّهُ عَلَى الْإِيمَانِ**

*O Allah, forgive our living and our dead, those present and those absent, our young and our old, our males and our females. O Allah, whoever You keep alive among us, let him live upon Islam, and whoever You cause to die, let him die upon faith.*

</div>

---

## 📚 Sources and References (المصادر والمراجع)

1. **Sahih Al-Bukhari** — Hadith: 1245, 1315, 1325
2. **Sunan Ibn Majah** — Hadith: 1458
3. **Al-Mudawwana** — Imam Malik
4. **Bidayat Al-Mujtahid** — Ibn Rushd

---

**And Allah knows best** — وَاللهُ تَعَالَى أَعْلَى وَأَعْلَم
`;
