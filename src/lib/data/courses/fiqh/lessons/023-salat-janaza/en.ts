/**
 * Fiqh - Lesson 023: صلاة الجنازة (Funeral Prayer)
 * English Content
 */

export const content = `# 📜 Fiqh of Special Prayers

## 📖 The Funeral Prayer (Salat Al-Janaza)

All praise be to Allah who created us, will cause us to die, then will resurrect us. May peace and blessings be upon our Prophet Muhammad ﷺ. The funeral prayer is a supplication for the deceased and a collective obligation on the community.

> The Prophet ﷺ said: **"Pray over your dead"** — Ibn Majah (1458)

<div class="scene-box">

**🎭 The Scene of the Two Qirat — The Merit of Accompanying Funerals**

*The companions were sitting with the Prophet ﷺ...*

**The Prophet ﷺ:** "Whoever follows a funeral until prayer is offered for it will have one qirat (of reward), and whoever follows it until burial will have two qirat"

**The Companions:** What are the two qirat?

**The Prophet ﷺ:** "Like **two immense mountains**"

</div>

**📍 Narrator:** Abu Hurayra رضي الله عنه — Al-Bukhari (1325)

---

## 🔍 1. Status and Importance

### 📌 Collective Obligation (فرض كفاية)

The funeral prayer is a **collective obligation**: if some perform it, the rest are absolved.

---

## 🔷 2. Description of the Prayer

### 📋 General Structure

| Element | Description |
|---------|-------------|
| **No ruku' or sujud** | Standing prayer only |
| **4 Takbirat** | With du'a between each |
| **Ends with taslim** | One taslim according to Malikis |
| **No adhan or iqama** | Like Eid prayer |

### 📋 The 4 Takbirat and what is recited

| Takbira | 🟡 What is recited (Maliki) |
|---------|---------------------------|
| **1st** | Opening du'a (recommended) |
| **2nd** | Salat upon the Prophet ﷺ |
| **3rd** | Du'a for the deceased |
| **4th** | General du'a, then taslim |

### ⚠️ MAJOR DIFFERENCE: Al-Fatiha

| School | Recitation of Fatiha |
|--------|---------------------|
| 🟡 **Maliki** | **No Fatiha** — Only du'a |
| 🟢 Hanafi | No Fatiha — Only du'a |
| 🔵 Shafi'i | **Fatiha obligatory** after 1st takbira |
| 🟣 Hanbali | **Fatiha obligatory** after 1st takbira |

> 💡 **Maliki Position:** The funeral prayer is essentially a supplication (du'a), not a regular prayer. Al-Fatiha is not recited in it.

---

## 📚 3. Du'as of the Funeral Prayer

### 📌 After the 2nd Takbira — Prayer upon the Prophet ﷺ

> **Allahumma salli 'ala Muhammad wa 'ala ali Muhammad, kama sallayta 'ala Ibrahim wa 'ala ali Ibrahim, innaka hamidun majid.**

### 📌 After the 3rd Takbira — Du'a for the deceased

> **Allahumma ighfir lahu warhamhu, wa 'afihi wa'fu 'anhu, wa akrim nuzulahu, wa wassi' mudkhalahu...**

---

## 🎯 4. Practical Rulings

### 📋 Imam's position relative to the deceased

| School | Man | Woman |
|--------|-----|-------|
| 🟡 **Maliki** | **Middle** | **Middle** |
| 🟢 Hanafi | Chest | Middle |
| 🔵 Shafi'i | Head | Middle |
| 🟣 Hanbali | Head | Middle |

### 📋 The Taslim

| School | Number of taslim |
|--------|------------------|
| 🟡 **Maliki** | **One taslim** to the right (slightly) |
| 🟢 Hanafi | One taslim |
| 🔵 Shafi'i | Two taslim |
| 🟣 Hanbali | Two taslim |

---

## ⚖️ 5. Prayer for the Absent (صلاة الغائب)

### ⚠️ Important difference

| School | Opinion |
|--------|---------|
| 🟡 **Maliki** | **Not legitimate** — Don't pray for absent |
| 🟢 Hanafi | Not legitimate |
| 🔵 Shafi'i | **Legitimate** for any Muslim who died |
| 🟣 Hanbali | Legitimate if no one prayed for him |

---

## 💡 6. Raising Hands

| School | Raising hands |
|--------|---------------|
| 🟡 **Maliki** | **Only at 1st takbira** |
| 🟢 Hanafi | Only at 1st takbira |
| 🔵 Shafi'i | At **every** takbira |
| 🟣 Hanbali | At **every** takbira |

---

## 🎯 Summary

| Point | Maliki Ruling |
|-------|---------------|
| **Status** | Fard Kifaya |
| **Takbirat** | 4 takbirat |
| **Fatiha** | ❌ No Fatiha |
| **Imam position** | At middle of body |
| **Taslim** | One to the right |
| **Raising hands** | Only 1st takbira |
| **Prayer for absent** | Not legitimate |

---

## 📚 Sources

1. Sahih Al-Bukhari, Hadiths no. 1245, 1315, 1325
2. Sunan Ibn Majah, Hadith no. 1458
3. Al-Mudawwana by Imam Malik
4. Mukhtasar Khalil
5. Bidayat Al-Mujtahid by Ibn Rushd
`;
