/**
 * Hajj - Leçon 007: Le Tawaf et le Sa'i (الطواف والسعي)
 * Contenu en Français - Conforme au guide de rédaction
 */

export const content = `# 📜 الطَّوَافُ وَالسَّعْيُ — Le Tawaf et le Sa'i

> Le Tawaf (circumambulation) autour de la Ka'ba et le Sa'i (parcours) entre Safa et Marwa sont parmi les rites les plus emblématiques du pèlerinage. **Le Tawaf al-Ifâda est un pilier du Hajj, tandis que le Sa'i en est également un pilier selon la majorité des savants.** Ces deux rites relient le pèlerin à l'histoire prophétique : le Tawaf rappelle la dévotion des prophètes depuis Ibrahim عليه السلام, et le Sa'i commémore la recherche d'eau de Hâjar عليها السلام pour son fils Ismâ'îl.

---

## 📖 Textes Coraniques Fondateurs

> **﴿ وَلْيَطَّوَّفُوا بِالْبَيْتِ الْعَتِيقِ ﴾**
> *« Et qu'ils accomplissent le Tawaf autour de l'Antique Maison »*
> — [Al-Hajj : 29]

> **﴿ إِنَّ الصَّفَا وَالْمَرْوَةَ مِن شَعَائِرِ اللَّهِ ۖ فَمَنْ حَجَّ الْبَيْتَ أَوِ اعْتَمَرَ فَلَا جُنَاحَ عَلَيْهِ أَن يَطَّوَّفَ بِهِمَا ﴾**
> *« Certes, Safa et Marwa sont parmi les rites d'Allah. Quiconque fait le Hajj ou la 'Umra, nulle faute à lui de les parcourir »*
> — [Al-Baqara : 158]

---

## 📜 Hadith Référence

> عَنْ جَابِرٍ رَضِيَ اللَّهُ عَنْهُ في وَصْفِ حَجَّةِ النَّبِيِّ ﷺ قَالَ: **« حَتَّى إِذَا أَتَيْنَا الْبَيْتَ مَعَهُ اسْتَلَمَ الرُّكْنَ فَرَمَلَ ثَلاَثًا وَمَشَى أَرْبَعًا، ثُمَّ نَفَذَ إِلَى مَقَامِ إِبْرَاهِيمَ فَقَرَأَ: ﴿ وَاتَّخِذُوا مِن مَّقَامِ إِبْرَاهِيمَ مُصَلًّى ﴾ فَصَلَّى رَكْعَتَيْنِ »**

D'après Jâbir رضي الله عنه décrivant le Hajj du Prophète ﷺ : **« Lorsque nous arrivâmes à la Maison avec lui, il toucha le Coin (la Pierre Noire) puis fit le Raml (marche rapide) pendant trois tours et marcha normalement pendant quatre. Puis il se dirigea vers le Maqâm d'Ibrahim et récita : ﴾ Et prenez le Maqâm d'Ibrahim comme lieu de prière ﴿ — puis il pria deux rak'at. »**

— [Muslim (1218)]

---

## 🎓 Paroles des Savants

| Savant | Ouvrage | Citation |
|--------|---------|----------|
| **L'Imam Mâlik** رحمه الله | *Al-Muwatta'* | « Le Tawaf al-Ifâda est le Tawaf obligatoire (du Hajj), c'est le Tawaf du rite (tawâf az-ziyâra), et sans lui le Hajj n'est pas complet. » |
| **Ibn Rushd** رحمه الله | *Bidâyat al-Mujtahid* | « Les savants sont unanimes sur l'obligation du Tawaf al-Ifâda. Ils divergent sur le Sa'i : la majorité en fait un pilier, les Hanafites le considèrent comme obligatoire (wâjib) mais non pilier. » |
| **Al-Qurtubî** رحمه الله | *Tafsîr* | « L'expression "nulle faute" fut révélée car certains Compagnons hésitaient à faire le Sa'i, pensant que c'était une pratique de la Jâhiliyya. Le verset confirme son obligation. » |
| **Khalîl ibn Ishâq** رحمه الله | *Al-Mukhtasar* | « Les piliers du Hajj sont quatre : l'Ihram, le Wuqûf, le Tawaf al-Ifâda, et le Sa'i. » |

---

## 🔍 Analyse Linguistique

| Terme Arabe | Translittération | Racine | Définition Technique |
|-------------|------------------|--------|---------------------|
| **طواف** | Tawâf | ط-و-ف | Tourner autour — circumambulation rituelle |
| **سعي** | Sa'y | س-ع-ي | Effort, marche — parcours entre Safa et Marwa |
| **رَمَل** | Ramal | ر-م-ل | Marche rapide à petits pas (sable) |
| **اضطباع** | Idtibâ' | ض-ب-ع | Découvrir l'épaule droite sous le Ridâ' |
| **استلام** | Istilâm | س-ل-م | Toucher, saluer — geste vers la Pierre Noire |
| **الحجر الأسود** | Al-Hajar al-Aswad | — | La Pierre Noire, angle sud-est de la Ka'ba |
| **الركن اليماني** | Ar-Rukn al-Yamânî | — | Le Coin Yéménite, angle sud-ouest |

---

## 📚 Explication Détaillée

### 1. Les Types de Tawaf

| Type de Tawaf | Moment | Statut | Description |
|---------------|--------|--------|-------------|
| **Tawaf al-Qudûm** | À l'arrivée à La Mecque | Sunna (🟡 Malikite) | Tawaf de bienvenue |
| **Tawaf al-Ifâda** | Après le Wuqûf d'Arafat (10+) | **Pilier** | Aussi appelé Tawaf az-Ziyâra |
| **Tawaf al-Wadâ'** | Avant de quitter La Mecque | Wâjib | Tawaf d'adieu |
| **Tawaf an-Nafl** | À tout moment | Recommandé | Tawaf surérogatoire |

### 2. Conditions de Validité du Tawaf

| Condition | Explication | Statut selon 🟡 Malikites |
|-----------|-------------|--------------------------|
| **L'intention (niyya)** | Formulée au début du Tawaf | **Condition** |
| **La pureté rituelle (tahâra)** | Wudû' exigé | **Condition de validité** |
| **Couverture de la 'awra** | Vêtements appropriés | **Condition** |
| **Sept tours complets** | Du Hajar Aswad au Hajar Aswad | **Condition** |
| **La Ka'ba à sa gauche** | Sens anti-horaire | **Condition** |
| **Être dans l'enceinte de la Mosquée** | Autour de la Ka'ba | **Condition** |
| **Continuité des tours** | Sauf interruption légale | **Condition** |

> Le Prophète ﷺ a dit : **« الطَّوَافُ بِالْبَيْتِ صَلاَةٌ، إِلاَّ أَنَّ اللَّهَ أَحَلَّ فِيهِ الْكَلاَمَ »**
> *« Le Tawaf autour de la Maison est une prière, sauf qu'Allah y a permis la parole. »*
> — [At-Tirmidhî (960), An-Nasâ'î]

### 3. Actes Sunna du Tawaf

#### 🟡 Al-Idtibâ' (découvrir l'épaule droite)

| Détail | Règle |
|--------|-------|
| **Quand** | Durant le Tawaf al-Qudûm et le Tawaf qui précède le Sa'i |
| **Pour qui** | Hommes uniquement |
| **Comment** | Passer le Ridâ' sous l'aisselle droite, sur l'épaule gauche |

#### 🟡 Ar-Ramal (marche rapide)

| Tours | Description |
|-------|-------------|
| **Tours 1-2-3** | Marche rapide à petits pas (ramal) |
| **Tours 4-5-6-7** | Marche normale |

> Le Prophète ﷺ « ramala » dans les trois premiers tours.
> — [Al-Bukhârî (1644), Muslim (1261)]

#### 🟡 Gestes envers la Pierre Noire et le Coin Yéménite

| Élément | Geste Optimal | Si Impossible |
|---------|---------------|---------------|
| **Pierre Noire** | Embrasser | Toucher avec la main → Faire signe de loin |
| **Coin Yéménite** | Toucher avec la main | Faire signe (divergence) |

> **﴿ رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الْآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ ﴾**
> *Entre le Coin Yéménite et la Pierre Noire, le Prophète ﷺ récitait cette invocation.*
> — [Abû Dâwûd (1892)]

### 4. Les Deux Rak'at après le Tawaf

> **﴿ وَاتَّخِذُوا مِن مَّقَامِ إِبْرَاهِيمَ مُصَلًّى ﴾**
> *« Et prenez le Maqâm d'Ibrahim comme lieu de prière »*
> — [Al-Baqara : 125]

| Détail | Règle |
|--------|-------|
| **Lieu idéal** | Derrière le Maqâm d'Ibrahim |
| **Si impossible** | N'importe où dans la Mosquée |
| **Sourates recommandées** | Al-Kâfirûn (1ère) et Al-Ikhlâs (2ème) |
| **Statut** | Sunna mu'akkada |

---

### 5. Le Sa'i : Définition et Conditions

Le Sa'i commémore la course de Hâjar عليها السلام cherchant de l'eau pour son fils Ismâ'îl. Elle monta sept fois sur Safa et Marwa, scrutant l'horizon. Allah fit jaillir la source de Zamzam.

| Condition du Sa'i | Explication |
|-------------------|-------------|
| **Intention** | Formulée au début |
| **Commencer par Safa** | Impératif |
| **Terminer à Marwa** | Le 7ème trajet finit à Marwa |
| **Sept trajets complets** | Safa → Marwa = 1, Marwa → Safa = 2, etc. |
| **Après un Tawaf valide** | Tawaf al-Qudûm ou Tawaf al-Ifâda |
| **En état d'Ihram** | Pour le Hajj ou la 'Umra |

> Le Prophète ﷺ a dit : **« نَبْدَأُ بِمَا بَدَأَ اللَّهُ بِهِ »**
> *« Nous commençons par ce par quoi Allah a commencé. »*
> — [Muslim (1218)]

### 6. Actes Sunna du Sa'i

| Acte | Description |
|------|-------------|
| **Monter sur Safa** | Jusqu'à voir la Ka'ba si possible |
| **Faire face à la Ka'ba** | Et lever les mains en invocation |
| **Dire le Takbîr** | « Allâhu Akbar » trois fois |
| **Réciter le Tahlîl** | « Lâ ilâha illa Allâh wahdahu... » |
| **Invoquer librement** | Répéter trois fois |
| **Marcher rapidement (Ramal)** | Entre les deux repères verts (pour les hommes) |
| **Même chose à Marwa** | Mêmes invocations |

---

## ⚖️ Points de vue des Écoles

| Aspect | 🟡 Malikite | 🟢 Hanafite | 🔵 Shafiite | 🟣 Hanbalite |
|--------|-------------|-------------|-------------|--------------|
| **Pureté pour le Tawaf** | **Condition** | Wâjib (non condition) | Condition | Condition |
| **Le Sa'i est-il un Pilier ?** | **Oui** | Non (wâjib) | Oui | Oui |
| **Pureté pour le Sa'i** | Recommandée | Recommandée | Recommandée | Recommandée |
| **Sa'i avant Tawaf** | **Invalide** | Invalide | Invalide | Valide (excuse) |

---

## 🌍 Applications Modernes

| Situation Contemporaine | Solution Jurisprudentielle |
|------------------------|---------------------------|
| **Tawaf en fauteuil roulant** | Valide — la capacité de marcher n'est pas condition |
| **Tawaf aux étages supérieurs** | Valide selon la majorité moderne |
| **Interruption pour la prière** | Reprendre du tour où on s'est arrêté |
| **Perte de Wudû' pendant le Tawaf** | 🟡 Recommencer ; d'autres : refaire le Wudû' et continuer |
| **Sa'i sur les étages du Mas'â** | Valide car fait partie du parcours légal |
| **Foule empêchant de toucher la Pierre** | Faire signe de loin (ishâra) suffit |

---

## ⚠️ Erreurs Courantes

| Erreur | Correction |
|--------|------------|
| Compter Safa-Marwa-Safa comme 1 tour | Safa → Marwa = 1, Marwa → Safa = 2 |
| Commencer par Marwa | Toujours commencer par Safa |
| Courir tout le Sa'i | Courir uniquement entre les deux repères verts |
| Invocations collectives à voix haute | Invoquer individuellement et silencieusement |
| Faire le Sa'i sans avoir fait de Tawaf | Le Tawaf doit précéder le Sa'i |
| Négliger les deux rak'at après le Tawaf | Sunna mu'akkada à ne pas délaisser |

---

## ❓ Questions Fréquentes (FAQ)

**Q1 : Puis-je faire le Tawaf sans Wudû' ?**
> 🟡 Selon les Malikites, non. Le Wudû' est une condition de validité. Selon les Hanafites, le Tawaf est valide mais avec fidya.

**Q2 : Combien de tours fait le Sa'i ?**
> 7 trajets : Safa → Marwa (1), Marwa → Safa (2), etc., jusqu'à finir à Marwa (7).

**Q3 : Que faire si je perds le compte des tours ?**
> Prendre le nombre le plus bas dont on est certain et continuer.

**Q4 : La femme doit-elle faire le Ramal ?**
> Non. Le Ramal est réservé aux hommes, tant pour le Tawaf que pour le Sa'i.

---

## 🎯 Cas Contemporains

### 🦽 Cas 1: Tawaf en fauteuil roulant — poussé par quelqu'un

> **Situation:** Une personne âgée ne peut pas marcher et fait le Tawaf en fauteuil roulant, poussée par son fils. Le fils a-t-il aussi accompli un Tawaf ? La personne âgée doit-elle avoir l'intention ?

**❓ Ce Tawaf est-il valide pour les deux ?**

<details>
<summary>📜 Voir la réponse</summary>

**📖 Analyse:**

| Critère | Application |
|---------|-------------|
| **Condition de marcher** | Non exigée — le Tawaf est valide assis |
| **Intention de la personne poussée** | Obligatoire pour elle |
| **Tawaf du pousseur** | Compte s'il en a l'intention |

> 💡 **Règle:** Le Tawaf en fauteuil roulant est **parfaitement valide**. La personne poussée doit avoir son intention propre. Le pousseur peut simultanément avoir l'intention de faire son propre Tawaf et aider, recevant ainsi double récompense.

</details>

### 📱 Cas 2: Réciter le Coran sur téléphone pendant le Tawaf

> **Situation:** Un pèlerin fait le Tawaf tout en lisant le Coran sur son téléphone. Il ne connaît pas d'invocations par cœur et préfère lire plutôt que de rester silencieux.

**❓ Cette pratique est-elle permise ?**

<details>
<summary>📜 Voir la réponse</summary>

**📖 Analyse:**

| Critère | Application |
|---------|-------------|
| **Récitation du Coran** | Permise pendant le Tawaf |
| **Utilisation du téléphone** | Outil de lecture, pas d'interdiction |
| **Mieux que le silence** | Oui, le Tawaf est prière (parole permise) |

> 💡 **Règle:** Réciter le Coran pendant le Tawaf est **permis et méritoire**. Le téléphone comme support de lecture n'altère pas le mérite. Cependant, il est préférable de mémoriser quelques invocations prophétiques et d'invoquer librement.

</details>

### 🤰 Cas 3: Femme enceinte fatiguée — pause prolongée pendant le Sa'i

> **Situation:** Une femme enceinte de 7 mois fait le Sa'i. À son 4ème trajet, elle est épuisée et doit s'asseoir pendant 45 minutes. Peut-elle reprendre là où elle s'est arrêtée ?

**❓ La continuité du Sa'i est-elle rompue ?**

<details>
<summary>📜 Voir la réponse</summary>

**📖 Analyse:**

| Critère | Application |
|---------|-------------|
| **Continuité du Sa'i** | Moins stricte que celle du Tawaf |
| **Pause pour repos** | Permise si justifiée |
| **Limite de temps** | Pas de limite stricte selon la majorité |

> 💡 **Règle:** Le Sa'i peut être interrompu pour une raison valable (fatigue, besoin physiologique). Elle reprend du trajet où elle s'est arrêtée. La grossesse est une excuse légale majeure. Il est conseillé d'utiliser un fauteuil roulant si disponible.

</details>

### 🚽 Cas 4: Interruption pour aller aux toilettes — reprendre où ?

> **Situation:** Au 5ème tour du Tawaf, un pèlerin a un besoin urgent d'aller aux toilettes. Il quitte le Tawaf, se soulage, refait le Wudû', puis revient. D'où reprend-il ?

**❓ Doit-il recommencer le Tawaf ou continuer ?**

<details>
<summary>📜 Voir la réponse</summary>

**📖 Analyse:**

| Critère | Application |
|---------|-------------|
| **Cause de l'interruption** | Besoin physiologique — excuse valable |
| **Wudû' renouvelé** | Condition pour reprendre (chez les Malikites) |
| **Point de reprise** | À partir du point d'arrêt |

> 💡 **Règle:** Selon les Malikites, si l'interruption est longue, il faut recommencer le Tawaf depuis le début. Selon d'autres écoles, on reprend du tour où on s'est arrêté après avoir refait le Wudû'. Par précaution, recommencer le tour en cours est préférable.

</details>

### 🌊 Cas 5: Tawaf pendant la pluie — glissade et chute

> **Situation:** Pendant un Tawaf sous la pluie, le sol devient glissant. Un pèlerin glisse et tombe à son 3ème tour. Il se relève mais est désorienté. Comment continuer ?

**❓ Doit-il recommencer le tour ou le Tawaf entier ?**

<details>
<summary>📜 Voir la réponse</summary>

**📖 Analyse:**

| Critère | Application |
|---------|-------------|
| **Interruption involontaire** | Ne rompt pas le Tawaf |
| **Orientation perdue** | Reprendre du point le plus probable |
| **En cas de doute** | Prendre le nombre le plus bas certain |

> 💡 **Règle:** La chute n'invalide pas le Tawaf. Le pèlerin se relève et reprend de là où il estime s'être arrêté. En cas de doute sur le nombre de tours, il prend le nombre inférieur dont il est certain (ex: doute entre 3 et 4 → compter 3).

</details>

---

## 💭 Réflexion

> *« Le Tawaf est une prière autour de la Maison d'Allah. Chaque pas est une invocation, chaque tour une élévation. »*

— Enseignement des savants

---

## ✅ Action Concrète / Défi

**Préparation pratique :**
1. **Mémorisez** l'invocation entre le Coin Yéménite et la Pierre Noire (Rabbanâ âtinâ...)
2. **Apprenez** à distinguer Idtibâ' et Ramal — pratiquez les gestes chez vous
3. **Visualisez** le parcours du Sa'i avec les repères verts sur une carte/vidéo

---

## 🤲 Invocation du Jour

> **لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ، وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ**

*« Il n'y a de divinité qu'Allah Seul, sans associé. À Lui la royauté et la louange, et Il est Omnipotent »*

*(À réciter sur Safa et Marwa, face à la Ka'ba)*

---

## 📚 Références

1. **Al-Muwatta'** — Imam Mâlik ibn Anas
2. **Bidâyat al-Mujtahid** — Ibn Rushd al-Hafîd
3. **Al-Mukhtasar** — Khalîl ibn Ishâq
4. **Tafsîr al-Qurtubî** — Imam Al-Qurtubî
5. **Sahîh Al-Bukhârî** — Imam Al-Bukhârî
6. **Sahîh Muslim** — Imam Muslim
7. **Sunan At-Tirmidhî** — Imam At-Tirmidhî

---

*Wa Allâhu a'lam* — واللَّهُ أَعْلَم
`
